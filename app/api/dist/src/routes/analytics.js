...
function isProbablyPublicIp(ip) {
    ...
    if (s.startsWith("172.")) {
        ...
        if (Number.isFinite(part) && part >= 16 && part <= 31)
            return false;
    }
    if (s.startsWith("169.254."))
        return false;
    if (s.startsWith("fe80:") || s.startsWith("fc") || s.startsWith("fd"))
        return false; // link-local + unique-local v6
    return true;
}
function pickFirstHeaderIp(v) {
    if (typeof v !== "string")
        return null;
...
function geoFromHeaders(req) {
    ...
    const country = (typeof headers["x-geo-country"] === "string" ? headers["x-geo-country"].trim() : null) ||
...
async function lookupGeo(ip) {
    ...
    try {
        let geo = { city: null, country: null };
...
analyticsRouter.post("/visit", async (req, res) => {
    ...
    try {
      ...
      `, [ipHash, country, city, userAgent]);
        let visitorId = result.rows[0]?.id ?? null;
        if (!visitorId) {
    ...
    try {
        ...
        if ((!city || !country) && visitor) {
            ...
            if (nextCity || nextCountry) {
                city = nextCity;
                country = nextCountry;
                await pool.query("UPDATE visitor_analytics SET city = COALESCE($1, city), country = COALESCE($2, country) WHERE ip_hash = $3", [city, country, ipHash]);
            }
        }
...
analyticsRouter.post("/link", requireAuth, async (req, res) => {
