...
adminRouter.get("/accounts", async (_req, res) => {
    ...
    try {
        const [usersRes, creatorsRes] = await Promise.all([
            pool.query(`SELECT id::text AS id, username, password, created_at, updated_at
           FROM app_accounts
          WHERE role = 'user'
          ORDER BY created_at DESC`),
            pool.query(`SELECT uuid::text AS id, username, password, temp_password, last_seen, created_at, updated_at
           FROM providers
          ORDER BY created_at DESC`),
        ]);
...
adminRouter.put("/accounts/users/:id", async (req, res) => {
    ...
    try {
        ...
        const rowCount = await pool.query(`UPDATE app_accounts SET ${setClauses.join(", ")} WHERE id = $1::uuid AND role = 'user'`, values);
...
adminRouter.delete("/accounts/users/:id", async (req, res) => {
    const pool = getPool();
    try {
        const rowCount = await pool.query(`DELETE FROM app_accounts WHERE id = $1::uuid AND role = 'user'`, [req.params.id]);
        if (rowCount.rowCount === 0)
            return res.status(404).json({ error: "not_found" });
        res.json({ ok: true });
    }
...
adminRouter.put("/accounts/creators/:id", async (req, res) => {
    ...
    try {
        ...
        if (tempPassword !== undefined) {
            values.push(tempPassword ?? null);
            setClauses.push(`temp_password = $${values.length}`);
        }
        const rowCount = await pool.query(`UPDATE providers SET ${setClauses.join(", ")} WHERE uuid = $1::uuid`, values);
        if (rowCount.rowCount === 0)
            return res.status(404).json({ error: "not_found" });
        res.json({ ok: true });
    }
...
const normalizeLinkUrl = (slot, value) => {
    if (slot === "bottom")
        return null;
...
adminRouter.post("/ads", async (req, res) => {
    ...
    try {
      ...
      `, [item.slot, cleanImage, cleanText, cleanLinkUrl]);
        await pool.query("INSERT INTO advertising_space_history (slot, image, text, link_url, action) VALUES ($1, $2, $3, $4, 'update')", [item.slot, cleanImage, cleanText, cleanLinkUrl]);
...
adminRouter.delete("/ads/:slot", async (req, res) => {
    ...
    if (!["home-1", "home-2", "home-3", "bottom"].includes(slot))
        return res.status(400).json({ error: "invalid_slot" });
    const pool = getPool();
    try {
        if (slot === "bottom") {
            await pool.query("UPDATE advertising_spaces SET image = NULL, text = 'Your Ads Here', link_url = NULL, updated_at = NOW() WHERE slot = 'bottom'");
            await pool.query("INSERT INTO advertising_space_history (slot, image, text, link_url, action) VALUES ('bottom', NULL, 'Your Ads Here', NULL, 'delete')");
      ...
      UPDATE advertising_spaces
       ...
       WHERE slot = $1
      `, [slot]);
        await pool.query("INSERT INTO advertising_space_history (slot, image, text, link_url, action) VALUES ($1, NULL, NULL, NULL, 'delete')", [slot]);
        return res.json({ ok: true });
    }
    catch {
