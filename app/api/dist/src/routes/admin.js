...
adminRouter.post("/ads", async (req, res) => {
    ...
    try {
      ...
      `, [item.slot, cleanImage, cleanText]);
        await pool.query("INSERT INTO advertising_space_history (slot, image, text, action) VALUES ($1, $2, $3, 'update')", [item.slot, cleanImage, cleanText]);
...
adminRouter.delete("/ads/:slot", async (req, res) => {
    ...
    try {
        if (slot === "bottom") {
            await pool.query("UPDATE advertising_spaces SET image = NULL, text = 'Your Ads Here', updated_at = NOW() WHERE slot = 'bottom'");
            await pool.query("INSERT INTO advertising_space_history (slot, image, text, action) VALUES ('bottom', NULL, 'Your Ads Here', 'delete')");
      ...
      UPDATE advertising_spaces
       ...
       WHERE slot = $1
      `, [slot]);
        await pool.query("INSERT INTO advertising_space_history (slot, image, text, action) VALUES ($1, NULL, NULL, 'delete')", [slot]);
        return res.json({ ok: true });
    }
    catch {
