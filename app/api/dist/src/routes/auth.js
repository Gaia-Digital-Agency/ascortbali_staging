...
authRouter.post("/login", rateLimit, async (req, res) => {
    ...
    try {
        ...
        `, [username]);
            ...
            const ok = password === String(creator.password ?? "") ||
...
function countMatches(input, account) {
    let matches = 0;
    if (input.name && account.names.some((v) => v && v === input.name))
        matches += 1;
    if (input.email && account.emails.some((v) => v && v === input.email))
        matches += 1;
    if (input.phoneNumber && account.phones.some((v) => v && v === input.phoneNumber))
        matches += 1;
    if (input.oldPassword && account.oldPasswords.some((v) => v && v === input.oldPassword))
        matches += 1;
    return matches;
}
...
authRouter.post("/change-password", requireAuth, async (req, res) => {
    ...
    try {
        ...
        `, [req.user.id]);
            ...
            const ok = currentPassword === String(creator.password ?? "") ||
...
authRouter.post("/forgot-password/verify", rateLimit, async (req, res) => {
    ...
    try {
        ...
        `);
            ...
            if (!matched)
                return res.status(401).json({ error: "invalid_recovery_data" });
