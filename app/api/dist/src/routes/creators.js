...
creatorsRouter.get("/:uuid", async (req, res) => {
    ...
    try {
        ...
        const creatorRes = await pool.query(`SELECT p.*
         FROM providers p
        WHERE p.uuid = $1::uuid`, [uuid]);
        if (creatorRes.rows.length === 0) {
            return res.status(404).json({ error: "Not found" });
        }
        // Query for all of the creator's images.
        const imagesRes = await pool.query(`SELECT image_id, image_file, sequence_number
         FROM provider_images
        WHERE provider_uuid = $1::uuid
        ORDER BY sequence_number ASC, image_id ASC`, [uuid]);
        return res.json({
